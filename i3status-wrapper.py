...
def get_volume(card='Master', left_bool=False):
    """ Get current volume. """
    line_no = 5 if left_bool else 6
    return os.popen("amixer get %s" % card).read().split('\n')[line_no].split()[4][1:-1]

def get_brightness():
    """ Get current Brightness. """
    return os.popen('xbacklight').read().split('.')[0]
